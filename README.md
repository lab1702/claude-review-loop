...
- Before committing, a check that fails without changing files gets one unchanged rerun per pass; if it then passes, it is reported as flaky instead of stopping the run. Checks rerun after a commit hook changes the commit get no such rerun, so any failure there stops the run.
