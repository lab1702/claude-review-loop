...
- Before committing, each pass allows one unchanged rerun of failed checks, used by the first check run with a failure that did not change files; if they then pass, they count as passing and are reported as flaky (or, when checks in that run also reformatted files, as passing after those changes) instead of stopping the run. A later failure in the same pass needs a verified repair or stops the run. The baseline gets its own rerun. Checks rerun after a commit hook changes the commit get no such rerun, so any failure there stops the run.
- No pushes, branch switches, amends, or history rewrites. Pushing is left to you; if a hook that a commit triggers would push, the run stops before any review.
